from Shared_Parking.tools.util import DBUtil,LogUtil,TimeUtil
import os
...
class TestReport:
    ...
    def write_result_db(self, result_info):
        ...
        now = TimeUtil.get_filename_time()
        sql = f'insert into {self.app_name}(version, module, type, ' \
              f'case_id, case_desc, test_result, execute_time, error_msg, error_screenshot) ' \
              f'values("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")' \
              %(result_info['version'], result_info['module'], result_info['type'],
                result_info['case_id'], result_info['case_desc'], result_info['test_result'],
                f'{now}', result_info['error_msg'], result_info['error_screenshot'])
        print(sql)
...
if __name__ == '__main__':
    # 1.写入数据库
    # result_info = {'version': 'v6.0', 'module': 'login', 'type': 'ui',
    #                'case_id': 'login-02', 'case_desc': '1.输入账号；2.输入密码；3.输入验证码；4.点击登录按钮',
    #                'test_result': 'test-pass', 'error_msg': '无', 'error_screenshot': '无'}
    # # TestReport('test_result').write_result_db(result_info)   #TestReport(表名)
