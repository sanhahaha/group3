...
class TestReport:
    ...
    def write_result_db(self, result_info):
        ...
        sql = f'insert into {self.app_name}(version, module, case_type, case_id,' \
              f'case_desc, case_result, finish_time, error_msg, error_screenshot)' \
              f'values("%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s") '%(result_info['version'], result_info['module'], result_info['case_type'],
    ...
    def generate_html_report(self, version):
        ...
        for result in all_result:
            ...
            result_str += f'<tr height="40">' \
                           f'<td width="7%">{result[0]}</td>' \
                           f'<td width="9%">{result[2]}</td>' \
                           f'<td width="10%">{result[3]}</td>' \
                           f'<td width="7%">{result[4]}</td>' \
                           f'<td width="20%">{result[5]}</td>' \
                           f'<td width="7%" bgcolor="{color}">{result[6]}</td>' \
                           f'<td width="15%">{result[7]}</td>' \
                           f'<td width="15%">{result[8]}</td>' \
                           f'<td width="10%">{screenshot}</td>' \
                           f'</tr>\r\n'

            # 替换模板上特定位置的字符串
            with open('../conf/template.html', encoding='utf-8') as file:
                contents = file.read()
...
if __name__ == '__main__':
    ...
    #                'case_result':'case-pass', 'error_msg':'无', 'error_screenshot':'无'}
    #
